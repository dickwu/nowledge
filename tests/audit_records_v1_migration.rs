...
impl AuditRecordsV1Backend for FakeBackend {
    ...
    async fn reconcile_existing_index(
        ...
        if index_uid != TARGET_INDEX_UID || primary_key != TARGET_PRIMARY_KEY || !apply_settings {
            bail!("audit_records_v1 attempted an unexpected index reconciliation");
        }
        ...
        if !state.inspection.exists {
            bail!("existing audit index disappeared; refusing empty recreation");
        }
        if state.converge_on_create {
            state.inspection = IndexInspection::ready();
        }
...
#[tokio::test]
async fn apply_creates_only_the_audit_records_index_and_is_idempotent_after_replanning() {
