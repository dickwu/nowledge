...
## Summary
Owner-aware alias for appending one history event without owner in the path.

## Handler
- Rust handler: `append_event_alias`
...
`persistence.status=partially_failed`. Retrying the same owner-scoped
`idempotency_key` reconciles only unfinished steps and replays the original
response with `duplicate=true` and the same operation id.
...
flowchart TD
  ...
  n2["Store.append_event_async resolves owner from request"]
  n3["Return event response"]
  n0 --> n1
  n1 --> n2
  n2 --> n3
```
