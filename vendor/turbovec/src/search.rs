...
#[cfg(all(target_arch = "x86_64", turbovec_avx512_stable))]
    ...
    let results = {
        ...
        let results: Vec<(Vec<f32>, Vec<i64>)> = (0..nq)
            ...
            .flat_map(|qi_start| {
                ...
                unsafe {
                    #[cfg(turbovec_avx512_stable)]
                    let used_avx512 = if !force_scalar
                    ...
                    #[cfg(not(turbovec_avx512_stable))]
                    let used_avx512 = false;

                    if !used_avx512 && !force_scalar && is_x86_feature_detected!("avx2") {
