...
fn ensure_verification_ready(ready: bool, failures: &[String]) -> Result<()> {
    ...
    bail!("operations_v1 verification failed: {reason}")
}

fn configured_admin() -> Result<MeiliAdmin> {
    let admin = MeiliAdmin::from_config(&Config::from_env());
    if !admin.configured() {
        bail!("RAG_MEILI_URL is required for operations_v1 maintenance");
    }
    Ok(admin)
}
...
fn usage() -> &'static str {
    "operations_v1 plan --out PLAN.json\n\
operations_v1 apply --plan PLAN.json [--dry-run]\n\
operations_v1 verify --plan PLAN.json"
}
...
fn fail(error: anyhow::Error) -> ! {
    ...
    eprintln!(
        ...
        serde_json::to_string_pretty(&envelope).unwrap_or_else(|_| {
            "{\"mode\":\"error\",\"migration\":\"operations_v1\",\"ok\":false}".to_string()
        })
    );
    process::exit(1)
}
...
mod tests {
    ...
    fn failed_verification_is_an_error_for_deployment_automation() {
        ...
        let error =
            ensure_verification_ready(false, &["required operations index is missing".to_string()])
                .unwrap_err();
        assert!(error
            .to_string()
            .contains("required operations index is missing"));
    }
}
