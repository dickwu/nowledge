...
pub const MIGRATION_NAME: &str = "audit_records_v1";
pub const ARTIFACT_SCHEMA_VERSION: u32 = 2;
pub const TARGET_INDEX_UID: &str = "rag_audit_records";
...
#[async_trait]
pub trait AuditRecordsV1Backend: Send + Sync {
...
#[async_trait]
impl AuditRecordsV1Backend for MeiliAdmin {
    async fn index_exists(&self, index_uid: &str) -> Result<bool> {
...
pub async fn create_plan<B: AuditRecordsV1Backend>(backend: &B) -> Result<MigrationPlan> {
...
pub async fn apply_plan<B: AuditRecordsV1Backend>(
    backend: &B,
    plan: &MigrationPlan,
    dry_run: bool,
) -> Result<ApplyReport> {
...
pub async fn verify_plan<B: AuditRecordsV1Backend>(
    backend: &B,
    plan: &MigrationPlan,
...
pub fn validate_plan(plan: &MigrationPlan) -> Result<()> {
    if plan.migration != MIGRATION_NAME || plan.schema_version != ARTIFACT_SCHEMA_VERSION {
        bail!("unsupported audit_records_v1 migration plan version");
    }
    if plan.index_uid != TARGET_INDEX_UID || plan.primary_key != TARGET_PRIMARY_KEY {
        bail!("audit_records_v1 may target only {TARGET_INDEX_UID} with primary key id");
    }
    if plan.destructive {
        bail!("audit_records_v1 refuses destructive migration plans");
    }
    if plan.observed_state == IndexState::PrimaryKeyMismatch {
        bail!("audit_records_v1 refuses plans for an index with an incompatible primary key");
    }
    match (plan.observed_state, plan.observed_created_at.as_deref()) {
        (IndexState::Missing, None) => {}
        (IndexState::Missing, Some(_)) => {
            bail!("audit_records_v1 missing-index plans cannot bind a createdAt generation")
        }
        (_, Some(created_at)) if !created_at.trim().is_empty() => {}
        _ => {
            bail!(
                "audit_records_v1 existing-index plans must bind a non-empty createdAt generation"
            )
        }
    }
    let desired_settings = settings_for(TARGET_INDEX_UID);
    ...
    {
        bail!("audit_records_v1 plan settings do not match this binary");
    }
    if plan.action != action_for(plan.observed_state) {
        bail!("audit_records_v1 plan action is inconsistent with its observed state");
    }
    if plan.plan_checksum != compute_plan_checksum(plan)? {
        bail!("audit_records_v1 migration plan checksum does not match its contents");
    }
    Ok(())
}

async fn inspect<B: AuditRecordsV1Backend>(backend: &B) -> Result<ObservedIndex> {
